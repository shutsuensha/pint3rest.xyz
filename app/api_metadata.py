...
#### 🔑 Основной функционал:
- **Пользователи**: регистрация, верификация почты, сброс пароля, JWT-аутентификация, загрузка изображений, обновление профиля.
- **Пины**: создание, удаление, поиск, загрузка медиа, управление сохранениями и лайками.
- **Теги**: управление тегами, поиск пинов по тегам.
- **Комментарии**: добавление, ответы, загрузка медиа, подсчет количества комментариев.
- **Лайки**: лайки для пинов и комментариев, проверка и подсчет.
- **Подписки**: управление подписками, получение списка подписчиков и подписок.
- **Чаты и сообщения**: создание чатов, изменение их параметров, отправка сообщений и медиа, просмотр истории.

#### 🛠️ Используемые технологии:
- **FastAPI** – для создания высокопроизводительного REST и GraphQL API.
- **FastAPI-Cache** – для кэширования на уровне API, улучшения производительности.
...
- **Redis** – для кэширования данных и ускорения работы, отзывов токенов, брокера и результатов Celery, а также для Celery RedBeat.
- **Celery** – для асинхронной обработки задач, включая отправку рекламы на почту через Celery Beat.
- **Celery[beat]** – для планирования задач (например, отправки email-рассылок).
- **Docker** – для контейнеризации приложений.
- **Docker Compose** – для управления многоконтейнерными приложениями.
- **Nginx** – для проксирования запросов и обеспечения безопасности.
- **SSL** – для безопасного соединения через HTTPS.
- **VPS** – для размещения приложения на виртуальном сервере.
- **Yandex S3** – для хранения и получения файлов.
- **httpx** – для взаимодействия с внешними API.
- **FastAPI-Mail** – для отправки email через FastAPI.
- **Websockets** – для поддержки двусторонней связи в реальном времени.
- **SSE (Server-Sent Events)** – для потоковой передачи данных с сервера на клиент.
...
- **Ruff** – для статического анализа кода и его оптимизации.
- **Alembic** – для миграций базы данных.
- **GitLab CI/CD** – для настройки автоматического деплоя и тестирования через CI/CD.
