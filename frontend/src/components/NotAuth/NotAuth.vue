...
<template>
  <ClipLoader v-if="loading" :color="color" :size="size"
    class="flex items-center justify-center h-screen transition font-extrabold" />
  <div v-else :class="`${bg} flex items-center justify-center h-screen transition-transform`">
    <div class="items-center space-y-6 flex flex-col">
      <div v-if="bg === 'bg-gray-100'" class="grid grid-cols-5 gap-4">
        <div v-for="(image, index) in images.slice(0, 5)" :key="index" class="p-2" data-aos="fade-up">
          <img :src="image" alt="Image" class="w-full h-80 rounded-3xl object-cover">
        </div>
      </div>
      <div v-if="bg === 'bg-red-100'" class="grid grid-cols-5 gap-4">
        <div v-for="(image, index) in images.slice(5, 10)" :key="index" class="p-2" data-aos="fade-up">
          <img :src="image" alt="Image" class="w-full h-80 rounded-3xl object-cover">
        </div>
      </div>
