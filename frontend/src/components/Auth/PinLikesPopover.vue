...
<template>
  <div @scroll="handleScroll"
    class="flex flex-col gap-4 bg-black shadow-2xl rounded-3xl text-sm  text-white z-30 h-72 w-60 overflow-y-auto">
    <RouterLink v-for="user in users" :key="user.id" :to="`/user/${user.username}`"
      class="flex items-center space-x-2 hover:underline cursor-pointer m-2">
