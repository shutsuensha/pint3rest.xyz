...
async def user_subscibe_user(db: db, user_id: user_id, user_id_to_follow: int):
    ...
    await db.execute(
        insert(SubsrciptionsOrm)
        .values(follower_id=user_id, following_id=user_id_to_follow)
    )
    await db.commit()
...
async def user_delete_subscibe_user(db: db, user_id: user_id, user_id_to_follow: int):
    ...
    await db.execute(
        delete(SubsrciptionsOrm)
        .where(SubsrciptionsOrm.follower_id == user_id, SubsrciptionsOrm.following_id == user_id_to_follow)
    )
    await db.commit()
...
async def check_user_follow_user(db: db, user_id: user_id, user_id_to_follow: int):
    ...
    is_following = await db.scalar(select(SubsrciptionsOrm).where(SubsrciptionsOrm.follower_id == user_id, SubsrciptionsOrm.following_id == user_id_to_follow))
    if is_following:
        return True
    else:
        return False
...
async def get_user_followers(db: db, user_id: user_id, id: int, filter: filter):
    ...
    stmt = (
        ...
        .where(SubsrciptionsOrm.following_id == id).order_by(SubsrciptionsOrm.id.desc()).offset(filter.offset).limit(filter.limit)
    )
...
async def get_cnt_user_followers(db: db, user_id: user_id, id: int):
    ...
    cnt_followers = await db.scalar(
        select(func.count()).select_from(SubsrciptionsOrm).where(SubsrciptionsOrm.following_id == id)
    )
...
async def get_user_following(db: db, user_id: user_id, id: int, filter: filter):
    ...
    stmt = (
        ...
        .where(SubsrciptionsOrm.follower_id == id).order_by(SubsrciptionsOrm.id.desc()).offset(filter.offset).limit(filter.limit)
    )
