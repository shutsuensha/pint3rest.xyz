...
async def create_like_on_pin(pin_id: int, user_id: user_id, db: db):
    ...
    like = await db.scalar(
        insert(LikesOrm)
        .values(user_id=user_id, pin_id=pin_id)
        .returning(LikesOrm)
    )
    await db.commit()
    return like
...
async def check_user_like_on_pin(pin_id: int, user_id: user_id, db: db):
    ...
    like = await db.scalar(select(LikesOrm).where(LikesOrm.user_id == user_id, LikesOrm.pin_id == pin_id))
    if like:
        return True
    else:
        return False
...
async def delete_like_on_comment(comment_id: int, user_id: user_id, db: db):
    ...
    await db.execute(delete(LikesOrm).where(LikesOrm.comment_id == comment_id, LikesOrm.user_id == user_id))
    await db.commit()
...
async def check_user_like_on_comment(comment_id: int, user_id: user_id, db: db):
    ...
    like = await db.scalar(select(LikesOrm).where(LikesOrm.user_id == user_id, LikesOrm.comment_id == comment_id))
    if like:
        return True
    else:
        return False
